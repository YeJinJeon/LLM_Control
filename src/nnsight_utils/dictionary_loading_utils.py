...
def load_gemma_sae(
    ...
    if "2b" in model_name:
        ...
        repo_id = "google/gemma-scope-9b-pt-" + (
            "res" if submod_type in ["embed", "resid"] else
            "att" if submod_type == "attn" else
            "mlp"
        )
...
def _load_gemma_saes_and_submodules(
    ...
    if "2b" in model_name:
        ...
        assert len(model.model.layers) == 42, "Not the expected number of layers for Gemma-2-9B"
    if thru_layer is None:
    ...
    if include_embed:
        ...
        dictionaries[embed] = load_gemma_sae(model_name, "embed", 0, neurons=neurons, dtype=dtype, device=device)
    else:
        embed = None
    for i, layer in tqdm(enumerate(model.model.layers[:thru_layer+1]), total=thru_layer+1, desc="Loading Gemma SAEs"):
        ...
        dictionaries[attn] = load_gemma_sae(model_name, "attn", i, neurons=neurons, dtype=dtype, device=device)
        mlps.append(
            mlp := Submodule(
                name=f"mlp_{i}",
        ...
        dictionaries[mlp] = load_gemma_sae(model_name, "mlp", i, neurons=neurons, dtype=dtype, device=device)
        resids.append(
            resid := Submodule(
                name=f"resid_{i}",
                submodule=layer,
        ...
        dictionaries[resid] = load_gemma_sae(model_name, "resid", i, neurons=neurons, dtype=dtype, device=device)
